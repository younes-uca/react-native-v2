...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<FieldDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new FieldCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<FieldDto[]>>(null);
    ...
    const fetchItems = (criteria: FieldCriteria) => {
        FieldAgentService.findPaginatedByCriteria(criteria).then(({data}) => {
