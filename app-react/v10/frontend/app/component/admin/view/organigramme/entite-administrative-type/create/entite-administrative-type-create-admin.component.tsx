...
const Create: React.FC<EntiteAdministrativeTypeCreateAdminType> = ({visible, onClose, add, showToast, list, t}) => {
    ...
    const saveItem = () => {
        setSubmitted(true);
        if (isFormValid()) {
            EntiteAdministrativeTypeAdminService.save(item).then(({data}) =>{
                add(data);
                MessageService.showSuccess(showToast, 'Entite administrative type Created');
                onClose();
                setSubmitted(false);
                });
        }
