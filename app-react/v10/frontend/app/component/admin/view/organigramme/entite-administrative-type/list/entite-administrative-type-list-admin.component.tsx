...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<EntiteAdministrativeTypeDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new EntiteAdministrativeTypeCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<EntiteAdministrativeTypeDto[]>>(null);
    ...
    const fetchItems = (criteria: EntiteAdministrativeTypeCriteria) => {
        EntiteAdministrativeTypeAdminService.findPaginatedByCriteria(criteria).then(({data}) => {
    ...
    const add = (item: EntiteAdministrativeTypeDto) => {
        setItems([...items, item]);
    };
