...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<RoleUtilisateurDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new RoleUtilisateurCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<RoleUtilisateurDto[]>>(null);
    ...
    const fetchItems = (criteria: RoleUtilisateurCriteria) => {
        RoleUtilisateurAdminService.findPaginatedByCriteria(criteria).then(({data}) => {
    ...
    const add = (item: RoleUtilisateurDto) => {
        setItems([...items, item]);
    };
