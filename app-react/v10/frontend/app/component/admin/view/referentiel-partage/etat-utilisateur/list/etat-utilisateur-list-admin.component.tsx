...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<EtatUtilisateurDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new EtatUtilisateurCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<EtatUtilisateurDto[]>>(null);
    ...
    const fetchItems = (criteria: EtatUtilisateurCriteria) => {
        EtatUtilisateurAdminService.findPaginatedByCriteria(criteria).then(({data}) => {
    ...
    const add = (item: EtatUtilisateurDto) => {
        setItems([...items, item]);
    };
