...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<DocumentTypeDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new DocumentTypeCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<DocumentTypeDto[]>>(null);
    ...
    const fetchItems = (criteria: DocumentTypeCriteria) => {
        DocumentTypeAdminService.findPaginatedByCriteria(criteria).then(({data}) => {
    ...
    const add = (item: DocumentTypeDto) => {
        setItems([...items, item]);
    };
