...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<TagDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new TagCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<TagDto[]>>(null);
    ...
    const fetchItems = (criteria: TagCriteria) => {
        TagAdminService.findPaginatedByCriteria(criteria).then(({data}) => {
