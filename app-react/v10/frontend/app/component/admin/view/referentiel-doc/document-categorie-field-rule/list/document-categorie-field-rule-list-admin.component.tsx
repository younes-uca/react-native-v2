...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<DocumentCategorieFieldRuleDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new DocumentCategorieFieldRuleCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<DocumentCategorieFieldRuleDto[]>>(null);
    ...
    const fetchItems = (criteria: DocumentCategorieFieldRuleCriteria) => {
        DocumentCategorieFieldRuleAdminService.findPaginatedByCriteria(criteria).then(({data}) => {
    ...
    const add = (item: DocumentCategorieFieldRuleDto) => {
        setItems([...items, item]);
    };
