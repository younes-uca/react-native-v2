...
const Create: React.FC<DocumentTypeCreateAdminType> = ({visible, onClose, add, showToast, list, t}) => {
    ...
    const saveItem = () => {
        setSubmitted(true);
        if (isFormValid()) {
            DocumentTypeAdminService.save(item).then(({data}) =>{
                add(data);
                MessageService.showSuccess(showToast, 'Document type Created');
                onClose();
                setSubmitted(false);
                });
        }
