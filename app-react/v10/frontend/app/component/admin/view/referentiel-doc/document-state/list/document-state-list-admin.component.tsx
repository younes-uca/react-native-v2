...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<DocumentStateDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new DocumentStateCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<DocumentStateDto[]>>(null);
    ...
    const fetchItems = (criteria: DocumentStateCriteria) => {
        DocumentStateAdminService.findPaginatedByCriteria(criteria).then(({data}) => {
    ...
    const add = (item: DocumentStateDto) => {
        setItems([...items, item]);
    };
