...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<DocumentCategorieDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new DocumentCategorieCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<DocumentCategorieDto[]>>(null);
    ...
    const fetchItems = (criteria: DocumentCategorieCriteria) => {
        DocumentCategorieAdminService.findPaginatedByCriteria(criteria).then(({data}) => {
    ...
    const add = (item: DocumentCategorieDto) => {
        setItems([...items, item]);
    };
