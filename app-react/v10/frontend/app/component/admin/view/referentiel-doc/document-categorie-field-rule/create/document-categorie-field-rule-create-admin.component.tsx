...
const Create: React.FC<DocumentCategorieFieldRuleCreateAdminType> = ({visible, onClose, add, showToast, list, t}) => {
    ...
    const saveItem = () => {
        setSubmitted(true);
        if (isFormValid()) {
            DocumentCategorieFieldRuleAdminService.save(item).then(({data}) =>{
                add(data);
