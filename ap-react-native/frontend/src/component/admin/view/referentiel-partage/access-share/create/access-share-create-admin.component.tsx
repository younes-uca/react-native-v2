...
const AccessShareAdminCreate = () => {

    const [showSavedModal, setShowSavedModal] = useState(false);
    const [showErrorModal, setShowErrorModal] = useState(false);
    const [isAccessShareCollapsed, setIsAccessShareCollapsed] = useState(true);
    ...
    const { control, handleSubmit, reset } = useForm<AccessShareDto>({
        defaultValues: {
        code: '' ,
        libelle: '' ,
        },
    ...
    const accessShareCollapsible = () => {
        setIsAccessShareCollapsed(!isAccessShareCollapsed);
    ...
    const handleSave = async (item: AccessShareDto) => {
        Keyboard.dismiss();
        try {
            await AccessShareAdminService.save( item );
            setIsItemsCollapsed(!isItemsCollapsed);
            reset();
            setShowSavedModal(true);
            setTimeout(() => setShowSavedModal(false), 1500);
        } catch (error) {
            console.error('Error saving accessShare:', error);
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create AccessShare< /Text>

            <TouchableOpacity onPress={accessShareCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
                <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 20 }}>AccessShare< /Text>
            </TouchableOpacity>

            <Collapsible collapsed={isAccessShareCollapsed}>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save AccessShare"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
...
export default AccessShareAdminCreate;
