...
const EtatUtilisateurAdminCreate = () => {

    const [showSavedModal, setShowSavedModal] = useState(false);
    const [showErrorModal, setShowErrorModal] = useState(false);
    const [isEtatUtilisateurCollapsed, setIsEtatUtilisateurCollapsed] = useState(true);
    ...
    const { control, handleSubmit, reset } = useForm<EtatUtilisateurDto>({
        defaultValues: {
        code: '' ,
        libelle: '' ,
        },
    ...
    const etatUtilisateurCollapsible = () => {
        setIsEtatUtilisateurCollapsed(!isEtatUtilisateurCollapsed);
    ...
    const handleSave = async (item: EtatUtilisateurDto) => {
        Keyboard.dismiss();
        try {
            await EtatUtilisateurAdminService.save( item );
            setIsItemsCollapsed(!isItemsCollapsed);
            reset();
            setShowSavedModal(true);
            setTimeout(() => setShowSavedModal(false), 1500);
        } catch (error) {
            console.error('Error saving etatUtilisateur:', error);
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create EtatUtilisateur< /Text>

            <TouchableOpacity onPress={etatUtilisateurCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
                <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 20 }}>EtatUtilisateur< /Text>
            </TouchableOpacity>

            <Collapsible collapsed={isEtatUtilisateurCollapsed}>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save EtatUtilisateur"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
...
export default EtatUtilisateurAdminCreate;
