...
const RoleUtilisateurAdminCreate = () => {

    const [showSavedModal, setShowSavedModal] = useState(false);
    const [showErrorModal, setShowErrorModal] = useState(false);
    const [isRoleUtilisateurCollapsed, setIsRoleUtilisateurCollapsed] = useState(true);
    ...
    const { control, handleSubmit, reset } = useForm<RoleUtilisateurDto>({
        defaultValues: {
        code: '' ,
        libelle: '' ,
        },
    ...
    const roleUtilisateurCollapsible = () => {
        setIsRoleUtilisateurCollapsed(!isRoleUtilisateurCollapsed);
    ...
    const handleSave = async (item: RoleUtilisateurDto) => {
        Keyboard.dismiss();
        try {
            await RoleUtilisateurAdminService.save( item );
            setIsItemsCollapsed(!isItemsCollapsed);
            reset();
            setShowSavedModal(true);
            setTimeout(() => setShowSavedModal(false), 1500);
        } catch (error) {
            console.error('Error saving roleUtilisateur:', error);
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create RoleUtilisateur< /Text>

            <TouchableOpacity onPress={roleUtilisateurCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
                <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 20 }}>RoleUtilisateur< /Text>
            </TouchableOpacity>

            <Collapsible collapsed={isRoleUtilisateurCollapsed}>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save RoleUtilisateur"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
...
export default RoleUtilisateurAdminCreate;
