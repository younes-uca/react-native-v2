...
const TagAdminCreate = () => {
    ...
    const { control, handleSubmit, reset } = useForm<TagDto>({
        defaultValues: {
        code: '' ,
        libelle: '' ,
        },
    ...
    const tagCollapsible = () => {
        setIsTagCollapsed(!isTagCollapsed);
    ...
    const handleSave = async (item: TagDto) => {
        Keyboard.dismiss();
        try {
            await TagAdminService.save( item );
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create Tag< /Text>

            <TouchableOpacity onPress={tagCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
                <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 20 }}>Tag< /Text>
            </TouchableOpacity>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save Tag"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
