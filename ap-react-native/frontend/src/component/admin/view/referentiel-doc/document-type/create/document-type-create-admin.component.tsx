...
const DocumentTypeAdminCreate = () => {
    ...
    const { control, handleSubmit, reset } = useForm<DocumentTypeDto>({
        defaultValues: {
        code: '' ,
        libelle: '' ,
        },
    ...
    const documentTypeCollapsible = () => {
        setIsDocumentTypeCollapsed(!isDocumentTypeCollapsed);
    ...
    const handleSave = async (item: DocumentTypeDto) => {
        Keyboard.dismiss();
        try {
            await DocumentTypeAdminService.save( item );
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create DocumentType< /Text>

            <TouchableOpacity onPress={documentTypeCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
                <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 20 }}>DocumentType< /Text>
            </TouchableOpacity>

            <Collapsible collapsed={isDocumentTypeCollapsed}>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save DocumentType"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
...
export default DocumentTypeAdminCreate;
