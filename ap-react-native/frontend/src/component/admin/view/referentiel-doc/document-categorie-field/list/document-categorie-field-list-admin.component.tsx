...
import  {DocumentCategorieFieldAdminCard}  from './DocumentCategorieFieldCard';


const DocumentCategorieFieldAdminList: React.FC = () =>  {
    ...
    const handleConfirmDelete = async () => {
        try {
            await DocumentCategorieFieldAdminService.deleteById(documentCategorieFieldId);
    ...
    const fetchData = async () => {
        try {
            ...
            DocumentCategorieFieldAdminService.getList(),
            ]);
    ...
    const handleFetchAndUpdate = async (id: number) => {
        try {
            const documentCategorieFieldResponse = await DocumentCategorieFieldAdminService.findById(id);
    ...
    <ScrollView showsVerticalScrollIndicator={false} style={{ paddingHorizontal: 10, backgroundColor: '#e6e8fa' }}>
        ...
        <View style={{ marginBottom: 100 }}>
            {documentCategorieFields && pdocumentCategorieFields.length > 0 ? ( documentCategorieFields.map((documentCategorieField) => (
                <DocumentCategorieFieldAdminCard key={documentCategorieField.id}
...
export default DocumentCategorieFieldAdminList;
