...
const DocumentStateAdminCreate = () => {

    const [showSavedModal, setShowSavedModal] = useState(false);
    const [showErrorModal, setShowErrorModal] = useState(false);
    const [isDocumentStateCollapsed, setIsDocumentStateCollapsed] = useState(true);
    ...
    const { control, handleSubmit, reset } = useForm<DocumentStateDto>({
        defaultValues: {
        code: '' ,
        libelle: '' ,
        style: '' ,
    ...
    const documentStateCollapsible = () => {
        setIsDocumentStateCollapsed(!isDocumentStateCollapsed);
    ...
    const handleSave = async (item: DocumentStateDto) => {
        Keyboard.dismiss();
        try {
            await DocumentStateAdminService.save( item );
            setIsItemsCollapsed(!isItemsCollapsed);
            reset();
            setShowSavedModal(true);
            setTimeout(() => setShowSavedModal(false), 1500);
        } catch (error) {
            console.error('Error saving documentState:', error);
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create DocumentState< /Text>

            <TouchableOpacity onPress={documentStateCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
                <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 20 }}>DocumentState< /Text>
            </TouchableOpacity>

            <Collapsible collapsed={isDocumentStateCollapsed}>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save DocumentState"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
...
export default DocumentStateAdminCreate;
