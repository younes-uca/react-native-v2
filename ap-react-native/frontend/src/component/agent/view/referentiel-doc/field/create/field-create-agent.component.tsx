...
const FieldAgentCreate = () => {

    const [showSavedModal, setShowSavedModal] = useState(false);
    const [showErrorModal, setShowErrorModal] = useState(false);
    const [isFieldCollapsed, setIsFieldCollapsed] = useState(true);
    ...
    const { control, handleSubmit, reset } = useForm<FieldDto>({
        defaultValues: {
        code: '' ,
        libelle: '' ,
        },
    ...
    const fieldCollapsible = () => {
        setIsFieldCollapsed(!isFieldCollapsed);
    ...
    const handleSave = async (item: FieldDto) => {
        Keyboard.dismiss();
        try {
            await FieldAgentService.save( item );
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create Field< /Text>

            <TouchableOpacity onPress={fieldCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
                <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 20 }}>Field< /Text>
            </TouchableOpacity>

            <Collapsible collapsed={isFieldCollapsed}>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save Field"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
...
export default FieldAgentCreate;
