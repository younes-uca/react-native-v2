...
import  {DocumentCategorieFieldRuleAgentCard}  from './DocumentCategorieFieldRuleCard';


const DocumentCategorieFieldRuleAgentList: React.FC = () =>  {
    ...
    const handleConfirmDelete = async () => {
        try {
            await DocumentCategorieFieldRuleAgentService.deleteById(documentCategorieFieldRuleId);
    ...
    const handleFetchAndUpdate = async (id: number) => {
        try {
            const documentCategorieFieldRuleResponse = await DocumentCategorieFieldRuleAgentService.findById(id);
    ...
    <ScrollView showsVerticalScrollIndicator={false} style={{ paddingHorizontal: 10, backgroundColor: '#e6e8fa' }}>
        ...
        <View style={{ marginBottom: 100 }}>
            {documentCategorieFieldRules && pdocumentCategorieFieldRules.length > 0 ? ( documentCategorieFieldRules.map((documentCategorieFieldRule) => (
                <DocumentCategorieFieldRuleAgentCard key={documentCategorieFieldRule.id}
...
export default DocumentCategorieFieldRuleAgentList;
