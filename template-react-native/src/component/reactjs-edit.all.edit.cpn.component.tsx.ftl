...
type ${pojo.name?cap_first}Edit${role.name?cap_first}Type = {
    visible: boolean,
    onClose: () => void,
    showToast: React.Ref<Toast>,
    selectedItem: ${pojo.name}Dto
    update: (item: ${pojo.name}Dto) => void,
    t: TFunction
}
const Edit: React.FC<${pojo.name?cap_first}Edit${role.name?cap_first}Type> = ({visible, onClose, showToast, selectedItem, update, t}) => {

    const emptyItem = new ${pojo.name}Dto();
    const [submitted, setSubmitted] = useState(false);
    const [activeIndex, setActiveIndex] = useState<number>(0);
    const [activeTab, setActiveTab] = useState(0);
    const [item, setItem] = useState<${pojo.name?cap_first}Dto>(selectedItem);
...
<#list pojo.fields as field>
    ...
    const add${field.name?cap_first} = () => {
        ...
        item.${field.name} = new Array<${field.typeAsPojo.name}Dto>();
        ...
        if (!_item.id) {
            ...
            const updatedItems = item.${field.name}.map((item) => item.id === ${field.name}.id ? {...${field.name}} : item);
    ...
    const delete${field.name?cap_first} = (rowData: any) => {
        ...
        set${field.name?cap_first}(new ${field.typeAsPojo.name}Dto());
        MessageService.showSuccess(showToast, '${pojo.name}Item Deleted');
    };
    ...
    const onMultiSelectChange${field.name?cap_first} = (e: any, field: string) => {
        if (e && e.value && Array.isArray(e.value)) {
            const selectedValues = e.value.map(option => option && option.value);
            set${field.name?cap_first}(prevState => ({ ...prevState, [field]: selectedValues, }));
        }
    };
    ...
    const editItem = async () => {
        ...
        if (isFormValid()) {
        <#list pojo.fields as field>
            <#if field.simple && field.dateTime>
            item.${field.name} = parseToIsoFormat(item.${field.name});
            </#if>
        </#list>
            const response = await ${pojo.name}${role.name?cap_first}Service.update(item);
            update(response.data);
            MessageService.showSuccess(showToast, '${pojo.name} Updated');
            onClose();
            setSubmitted(false);
    }
};
            ...
            <div className="grid">
                <#list myStep.fields as field>
                    ...
                    <#elseif field.generic && !field.notVisibleInCreatePage>
                        <div className="field col-11 md:col-<#if field.typeAsPojo.ignoreFront == false>${field.columnStyle}<#else >${field.columnStyle}</#if>">
                            <label for="${field?index}">${field.name18nCreate!'walo'}<#if field.requierd><span style="color: red;">*</span></#if></label>
                        ...
                        <#if !field.hidePlusButton>
                            <div className="col-1">
                                <label></label>
                                <button pButton pRipple type="button" icon="pi pi-plus" (click)="openCreate${field.name?cap_first}('${field.name?cap_first}')" className="p-button-success one-colonne-btn-success-align"></button>
                            </div>
                        </#if>
                    </#if>
                </#list>
            ...
            <TabPanel header=${pojo.tabPanelI18nCreate}>
                <div className="formgrid grid">
                    ...
                    <div className="field col-6">
                        ...
                        <Calendar id="${field.name}" value={adaptDate(item?.${field.name})} onChange={(e) => onInputDateChange(e, '${field.name}')} dateFormat="dd/mm/yy" showTime />
                    </div>
                    <#elseif  field.large>
                    <div className="field col-6">
                        <label htmlFor="${field.name}">${field.name18nCreate}</label>
                    ...
                    <TabPanel header="Creation">
                        ...
                        <div className="grid">
                            ...
                            <div className="field col-6">
                                ...
                                <Calendar id="${innerField.name}" value={adaptDate(${field.name}?.${innerField.name})}  onChange={(e) => onInputDateChange${field.name?cap_first}(e, '${innerField.name?uncap_first}')} dateFormat="dd/mm/yy" showTime  />
                            </div>
                            ...
                            <div className="field col-6">
                                ...
                                <InputNumber id="${innerField.name}" value={${field.name}.${innerField.name}}  onValueChange={(e) => onInputNumerChange${field.name?cap_first}(e, '${innerField.name?uncap_first}')}/>
                            </div>
                            <#elseif innerField.generic && innerField.typeAsPojo.name != pojo.name>
                            <div className="field col-6">
                                <label htmlFor="${innerField.name}">${innerField.name18nCreate}</label>
                                <Dropdown id="${innerField.name?uncap_first}Dropdown" value={${field.name}.${innerField.name}} options={${innerField.name?uncap_first}s} onChange={(e) => onDropdownChange${field.name?cap_first}(e, '${innerField.name?uncap_first}')} placeholder="Sélectionnez un ${innerField.name}" filter  filterPlaceholder="Rechercher un ${innerField.name?uncap_first}"  optionLabel="${innerField.typeAsPojo.labelOrReferenceOrId.name!'walo'}" />
                            </div>
                            ...
                            <DataTable value={item.${field.name?uncap_first}} tableStyle={{minWidth: '50rem'}} dataKey="id">
                                ...
                                <Column header="Actions" body={(rowData) => (
                                    <div>
                                        ...
                                        <Button icon="pi pi-pencil" rounded severity="success" className="mr-2" onClick={()=> edit${field.name?cap_first}(rowData)} />
                                    </div>
                                )}></Column>
                            </DataTable>
                        </div>
