...
</#if>import {Dialog} from 'primereact/dialog';
...
import {${pojo.name}Criteria} from "app/controller/criteria/${pojo.name}Criteria.model";
...
type ${pojo.name?cap_first}Create${role.name?cap_first}Type = {
    ...
    list: ${pojo.name}Dto[],
    service: ${pojo.name}${role.name?cap_first}Service,
    t: TFunction
}
const Create: React.FC<${pojo.name?cap_first}Create${role.name?cap_first}Type> = ({visible, onClose, add, showToast, list, service, t}) => {
    ...
    const emptyItem = new ${pojo.name}Dto();
        const {
            ...
            saveItem,
            formateDate
        } = useCreateHook<${pojo.name?cap_first}Dto, ${pojo.name?cap_first}Criteria>({list, emptyItem, onClose, add, showToast,service})
