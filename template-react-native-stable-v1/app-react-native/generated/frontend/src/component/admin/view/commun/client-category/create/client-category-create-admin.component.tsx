...
const ClientCategoryAdminCreate = () => {

    const [showSavedModal, setShowSavedModal] = useState(false);
    const [showErrorModal, setShowErrorModal] = useState(false);
    const [isClientCategoryCollapsed, setIsClientCategoryCollapsed] = useState(true);
    ...
    const { control, handleSubmit, reset } = useForm<ClientCategoryDto>({
        defaultValues: {
        reference: '' ,
        code: '' ,
        },
    ...
    const handleSave = async (item: ClientCategoryDto) => {
        Keyboard.dismiss();
        try {
            ...
            console.error('Error saving clientCategory:', error);
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create ClientCategory</Text>

            <TouchableOpacity onPress={clientCategoryCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
                <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 20 }}>ClientCategory</Text>
            </TouchableOpacity>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save ClientCategory"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
...
export default ClientCategoryAdminCreate;
