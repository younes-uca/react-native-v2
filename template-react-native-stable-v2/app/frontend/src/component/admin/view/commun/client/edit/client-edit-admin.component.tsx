...
const ClientAdminEdit: React.FC<Props> = ({ route }) => {
    ...
    const handleUpdate = async (item: ClientDto) => {
        ...
        try {
            await clientAdminService.update(item);
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }}>
        ...
        item.clientCategory = selectedClientCategory;
        <FilterModal visibility={clientCategoryModalVisible} placeholder={"Select a ClientCategory"} onItemSelect={onClientCategorySelect} items={clientCategorys} onClose={handleCloseModal} variable={'reference'} />
